...
async function extractSchemas (jsonStringInput, contents) {
  const schemas = [];
  const schema = {
    path: '',
    httpVerb: '',
    ...
    contentType: '',
    schemaName: '',
    objectType: '', // array or object
    schemaObj: {}
  };

  var jsonInput = JSON.parse(jsonStringInput);
  ...
  contents.map(content => {
    for (const reference in content.contentObj.schema) {
      if (content.contentObj.schema.hasOwnProperty(reference)) {
        ...
        schema.schemaName = content.contentObj.schema;
        schema.objectType = 'array';
        schema.schemaObj = {};
      }
    }
  });
...
async function extractComponents (jsonStringInput, schemas) {
  return null;
}