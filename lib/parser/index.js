...
async function _verifyOpenApiVersion(jsonStringInput) {
...
async function _verifyIfServerExist(jsonStringInput) {
  let result = false;
...
async function _verifyIfPathsExist(jsonStringInput) {
  let pathsKeys = await _extractPaths(jsonStringInput);

  return pathsKeys.length > 0;
}

async function _verifyIfPathHasHttpVerb(jsonStringInput) {
  // if one given http path has no verbs, parser cannot work.

  let jsonOutput = JSON.parse(jsonStringInput);

  let pathKeys = await _extractPaths(jsonStringInput);
...
async function _verifyIfPathReferencesContent(jsonStringInput) {
...
async function _verifyIfContentReferencesSchema(jsonStringInput) {
  return false;
}

async function _extractPaths(jsonStringInput) {
...
async function _extractHttpMethods(pathkeys, jsonStringInput) {
  const jsonOutput = JSON.parse(jsonStringInput);
  const httpsMethods = [];
...
async function _extractResponses(pathkeys, jsonStringInput, httpMethods) {
  const responses = [];
...
async function _extractContent(pathkeys, jsonStringInput, httpMethods, responses) {
  const contents = [];
  responses.map()
  return contents;
}

async function _extractSchemas(pathkeys, jsonStringInput, httpMethods, responses, contents){
